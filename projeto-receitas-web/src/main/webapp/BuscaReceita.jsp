...
<body>
	...
	<div class="container">
		...
		<input type="hidden" name="receitas" value="${receitas}"/>

		<jsp:useBean id="daoreceita" class="dao.ReceitaDao" />
		<c:set var="listadereceitas" value="${receitas}" />
			...
			<table class="table">
				<thead>
					<tr
						...
						<th
							style="color: white; font-weight: 500; font-size: 15px; font-family: 'Poppins'"
							scope="col">T�tulo</th>
						<th
							style="color: white; font-weight: 500; font-size: 15px; font-family: 'Poppins'"
				...
				<tbody>
					...
					<c:forEach var="receita" items="${listadereceitas}">
						<tr style="background-color: #e5e4e2;">
							<td scope="row"><a
								href="${pageContext.request.contextPath}/exibicaoreceita?action=exibir&id=${receita.id}">${receita.titulo}</a></td>
